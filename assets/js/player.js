...
// como SVG simples. Se guarda el token ('mustache'/'beard'), no el dibujo, en la base de datos.
const FACIAL_HAIR_LIST = ['mustache', 'beard'];
...
/** Devuelve el HTML a mostrar para un valor de complemento: emoji tal cual, o el SVG si es vello facial */
function glyphHTML(key, value, sizePx) {
  ...
  if (key === 'facial_hair') {
    ...
    if (!svg) return '';
    const s = sizePx || 24;
    return `<span style="display:inline-flex;width:${s.toFixed ? s.toFixed(1) : s}px;height:${(s*0.6).toFixed(1)}px;align-items:center;justify-content:center">${svg}</span>`;
  }
  return value;
}
...
const ACCESSORY_SPECS = {
  ...
  hat:         { fontPct: 0.42, z: 4, defTop: 8, defLeft: 50 },
...
function renderQuestion(state) {
  ...
  if (isMeme) loadMemePlayer('q-meme-img', currentSong.youtube_id, currentSong.start_seconds);
    ...
    if (i < n) {
      ...
      card.innerHTML = song.youtube_id
        ? `<div class="ts-year">${song.year}</div>
           ...
           <div class="ts-info"><div class="ts-title">${esc(song.title || '(sin título)')}</div></div>`
        : `<div class="ts-year">${song.year}</div>
           <div class="ts-info">
...
function renderResults(state) {
  ...
  if (isMeme) loadMemePlayer('r-meme-img', song.youtube_id, song.start_seconds);
  ...
  timeline.forEach((s, i) => {
    ...
    card.innerHTML = s.youtube_id
      ? `<div class="ts-year">${s.year}</div>
         ...
         <div class="ts-info"><div class="ts-title">${esc(s.title || '(sin título)')}</div></div>`
      : `<div class="ts-year">${s.year}</div>
         <div class="ts-info">
